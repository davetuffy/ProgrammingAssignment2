...
## cacheSolve takes the list of functions generated by makeCacheMatrix and uses it to invert the matrix
## that was sent to makeCacheMatrix and store it in cache.  If the inverted matrix is already in cache
## it grabs the value from the store.

cacheSolve <- function(x, ...) {
## The orginal comment in the code state 'Return a matrix that is the inverse of 'x'
## Note that this is only true for the value of x in the environment enclosure created by makeCacheMatrix
## Since this comment is contained in the body of the function cacheSolve, it would be logical to assume
## that it refers the value of x in cacheSolve.  Grammar has syntax and lexial rules too.  ;)
        iMat <- x$getinverted()
        if(!is.null(iMat)) {
